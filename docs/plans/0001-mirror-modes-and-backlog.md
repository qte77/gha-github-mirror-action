...
| A3 `ci/shellcheck-actionlint` | `.github/workflows/lint.yaml` (new), minimal shellcheck-driven fixes in `scripts/mirror.sh` (`\|\| exit 2` guard + 2 inline disable comments), README Development | — (merges before A4; A4 rebases) — **DONE, PR #47, CI green** |
...
| A3 | Add shellcheck + actionlint to CI. Prefer no marketplace-action pin where a direct install works (smaller supply-chain surface): `sudo apt-get install -y shellcheck` if not already preinstalled on `ubuntu-latest` (fetch/confirm live) and `actionlint` via its official install script or a pinned `docker run rhysd/actionlint:<tag>` (fetch current tag live — **UNVERIFIED until fetched**, don't guess a version); fix findings in `scripts/*.sh`, `.github/scripts/*.sh` — expect at least `cd "$CLONE_DIR"` without `\|\| exit` (SC2164) at mirror.sh:71 (moot once A4's `trap` lands — sequence A3 first, then A4 rebases and re-checks); README Development lists the local commands | agent | `ci/shellcheck-actionlint` | **SHIPPED — PR #47.** CI was 10/10 green on the substantive commit; `shellcheck` preinstalled (`0.9.0-1`), no install step; `actionlint` installed via its own download script pinned to `v1.7.12` (current stable, fetched via `gh api`). SC2164 at mirror.sh:71 was superseded by A4's `git -C` rewrite (no `cd` left to guard) — resolved on rebase onto A4 by taking A4's version; the two SC2001 disable-comments A3 added on the old sed-based PAT lines are likewise gone (A4 replaced those lines with bash substitution). |
